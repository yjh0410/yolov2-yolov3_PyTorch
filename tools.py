...
def set_anchors(anchor_size, stride):
    """
    Input:
...
def generate_txtytwth(gt_label, w, h, s, all_anchor_size):
    ...
    anchor_boxes = set_anchors(all_anchor_size, s)
        ...
        for gt_label in label_lists[batch_index]:
            ...
            if results:
                for result in results:
                    ...
                    if grid_x < gt_tensor.shape[1] and grid_y < gt_tensor.shape[2]:
...
def multi_gt_creator(input_size, strides, scale_thresholds, num_classes, label_lists=[], name='VOC'):
    ...
    # generate gt datas
    all_anchor_size = get_total_anchor_size(input_size, multi_scale=True, name=name)
    anchor_number = len(all_anchor_size[0])
    for s in strides:
    ...
    for batch_index in range(batch_size):
        for gt_label in label_lists[batch_index]:
            ...
            for index, s in enumerate(strides):
                results = generate_txtytwth(gt_label, w, h, s, all_anchor_size[index])
                if not results:
                    continue
                else:
                    max_iou_index, grid_x, grid_y, tx, ty, tw, th, weight = results
                    gt_tensor[index][batch_index, grid_y, grid_x, max_iou_index, 0] = 1.0
                    gt_tensor[index][batch_index, grid_y, grid_x, max_iou_index, 1] = gt_class
                    gt_tensor[index][batch_index, grid_y, grid_x, max_iou_index, 2:6] = np.array([tx, ty, tw, th])
                    gt_tensor[index][batch_index, grid_y, grid_x, max_iou_index, 6] = weight
                    # break
    gt_tensor = [gt.reshape(batch_size, -1, 1+1+4+1) for gt in gt_tensor]
    gt_tensor = np.concatenate(gt_tensor, 1)
    return gt_tensor

def loss(pred, label, num_classes, strides=None, input_size=None, use_focal=False):
